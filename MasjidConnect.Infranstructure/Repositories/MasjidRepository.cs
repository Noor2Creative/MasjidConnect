    ...
    public class MasjidRepository : IMasjidRepository
    {
        ...
        public async Task<ResponseHeader?> RegisterNamazTimeAsync(NamazTimeRequest model)
        {
            ...
            try
            {
                ...
                var parameters = new[]
                {
                    new SqlParameter("@RecId", model.Id),
                    new SqlParameter("@MasjidId", model.MasjidId),
                    new SqlParameter("@Fajr", model.Fajr?? (object)DBNull.Value),
                    new SqlParameter("@Dhuhr", model.Dhuhr),
                    new SqlParameter("@Asr", model.Asr ?? (object)DBNull.Value),
                    new SqlParameter("@Maghrib", model.Maghrib),
                    new SqlParameter("@Isha", model.Isha),
                    new SqlParameter("@JumaKhutba", model.JumaKhutba),
                    new SqlParameter("@EnteredBy", model.EnteredBy),
                };

                DataTable result = await _dbExecuter.ExecuteProcedureDtAsync(ProcedureManager.InsertOrUpdateNamazTime, parameters);
                if (result != null && result.Rows.Count > 0)
                {
                    string flag = result.Rows[0]["flag"].ToString().ToLower();
                    if (flag == "ok")
